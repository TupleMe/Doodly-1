package tuple.me.lily.views.fastscroll;
...
import tuple.me.lily.views.fastscroll.viewprovider.DefaultScrollerViewProvider;
import tuple.me.lily.views.fastscroll.viewprovider.ScrollerViewProvider;
...
public class FastScroller extends LinearLayout {
    ...
    private float getRelativeTouchPosition(MotionEvent event) {
        if (isVertical()) {
            float yInParent = event.getRawY() - ViewUtils.getViewYPos(handle);
            return yInParent / (getHeight() - handle.getHeight());
        } else {
            float xInParent = event.getRawX() - ViewUtils.getViewYPos(handle);
    ...
    private void setRecyclerViewPosition(float relativePos) {
        ...
        int targetPos = (int) RangeUtils.getValueInRange(0, itemCount - 1, (int) (relativePos * (float) itemCount));
